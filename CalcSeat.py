...
#simulates boarding using a sort by seat entrance method
def SeatSim():
    ...
    def genRowCol(bottom, top):
        conflict = True
        coord = [0, 0] #list as [row, col]

        while(conflict):
            coord[0] = bottom if int(random.randint(0, 1)) % 2 == 0 else top
    ...
    for x in range (65):
        manifest.append(Passengers.Passengers(genRowCol(1, 6), genNumBags()))
    for x in range(65):
        manifest.append(Passengers.Passengers(genRowCol(2, 5), genNumBags()))
    for x in range(65):
        manifest.append(Passengers.Passengers(genRowCol(3, 4), genNumBags()))
